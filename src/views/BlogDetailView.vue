...
// 强制刷新当前文章数据
const refreshCurrentPost = async (automatic = false) => {
  if (!route.params.id) return;

  // 如果是自动刷新（页面加载/激活时），不显示加载状态和消息
  if (!automatic) {
  ...
  try {
    ...
    if (!existingPost) {
      // 如果文章列表中没有找到，才单独获取文章
      await blogStore.fetchPost(route.params.id, false); // 不强制刷新，避免通知
    }
...
// 初始加载和每次激活页面时刷新数据（自动模式）
onMounted(() => refreshCurrentPost(true));
onActivated(() => refreshCurrentPost(true));
