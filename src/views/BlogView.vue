...
const handleFileChange = (event) => {
    ...
    if (files.length > 0) {
        ...
        if (file.name.endsWith('.md') || file.type === 'text/markdown') {
            ...
            uploadStatus.value = { loading: false, message: '请选择Markdown (.md)文件', isError: true }
        }
    }
}
...
const handleMultipleFilesChange = (event) => {
    ...
    if (files.length > 0) {
        ...
        if (mdFiles.length === 0) {
            uploadStatus.value = { loading: false, message: '没有找到Markdown文件', isError: true }
            return
        }

        batchFiles.value = mdFiles
        uploadStatus.value = { loading: false, message: `已选择${mdFiles.length}个Markdown文件`, isError: false }
    }
}
...
const handleUpload = async () => {
    if (!uploadedFile.value) {
        uploadStatus.value = { loading: false, message: '请先选择Markdown文件', isError: true }
        return
    }
    ...
    try {
        ...
        // 更新状态
        uploadStatus.value = { loading: false, message: '博客上传成功！', isError: false }
        ...
        uploadStatus.value = { loading: false, message: `上传失败: ${error.message}`, isError: true }
    }
}
...
const handleBatchUpload = async () => {
    ...
    uploadStatus.value = { loading: true, message: '开始批量上传...', isError: false }
            ...
            <div>
                <div class="flex justify-between items-center mb-6">
                    ...
                    <div class="flex gap-2">
                        <button @click="switchToBatchUpload" v-if="!showUploadForm && !showBatchUploadForm"
                            class="px-3 py-2 bg-github-green text-white rounded-md hover:bg-green-700 flex items-center">
                        ...
                        <button @click="switchToSingleUpload" v-if="!showUploadForm && !showBatchUploadForm"
                            class="px-3 py-2 bg-github-blue text-white rounded-md hover:bg-blue-700 flex items-center">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-1" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M12 4v16m8-8H4" />
                        ...
                        <button @click="cancelUpload" v-if="showUploadForm"
                            class="px-3 py-2 bg-gray-500 text-white rounded-md hover:bg-gray-700 flex items-center">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-1" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <button @click="cancelBatchUpload" v-if="showBatchUploadForm"
                            class="px-3 py-2 bg-gray-500 text-white rounded-md hover:bg-gray-700 flex items-center">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-1" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div v-if="uploadStatus.message" class="mb-4">
                    <div :class="[
                        'p-3 rounded-md',
                        uploadStatus.isError ? 'bg-red-100 text-red-700 dark:bg-red-900 dark:text-red-300' : 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300'
                    ]">
                        <div class="flex items-center">
                            <svg v-if="uploadStatus.isError" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2"
                        ...
                        <!-- 进度条 -->
                        <div v-if="uploadStatus.loading && batchUploadProgress.total > 0" class="mt-2">
                            <div class="h-2 bg-gray-300 dark:bg-gray-600 rounded-full overflow-hidden">
                                <div class="h-full bg-github-blue dark:bg-blue-500"
                ...
                <div v-if="showUploadForm"
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <div class="flex items-center">
                                <label for="markdown-file"
                                    class="flex-1 cursor-pointer border border-dashed border-[var(--color-border)] rounded-md p-4 text-center hover:border-github-blue transition-colors">
                                    <input type="file" id="markdown-file" class="hidden" accept=".md,text/markdown"
                                        @change="handleFileChange" />
                                    <div v-if="!uploadedFile" class="flex flex-col items-center">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-10 w-10 text-github-gray mb-2"
                                            fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
                                        </svg>
                                        <span class="text-github-gray">点击上传或拖放Markdown文件</span>
                                    </div>
                        ...
                        <div>
                            ...
                            <input type="text" id="category" v-model="newCategory" placeholder="分类名称"
                                class="w-full p-2 border border-[var(--color-border)] rounded-md bg-[var(--color-bg-primary)]" />
                            <div class="mt-1 flex flex-wrap gap-2" v-if="blogStore.categories.length">
                                <span class="text-xs">现有分类:</span>
                                <button v-for="category in blogStore.categories" :key="category"
                        ...
                        <div>
                            ...
                            <input type="text" id="tags" v-model="newTags" placeholder="标签1, 标签2, 标签3"
                                class="w-full p-2 border border-[var(--color-border)] rounded-md bg-[var(--color-bg-primary)]" />
                            <div class="mt-1 flex flex-wrap gap-2" v-if="blogStore.tags.length">
                                <span class="text-xs">现有标签:</span>
                                <button v-for="tag in blogStore.tags" :key="tag"
                                    @click="newTags ? (newTags.includes(tag) ? null : newTags += `, ${tag}`) : newTags = tag"
                        ...
                        <div class="flex justify-end gap-2 mt-6">
                            ...
                            <button @click="handleUpload" :disabled="!uploadedFile || uploadStatus.loading" :class="[
                                'px-4 py-2 rounded-md text-white',
                                !uploadedFile || uploadStatus.loading ? 'bg-gray-400 cursor-not-allowed' : 'bg-github-blue hover:bg-blue-700'
                            ]">
                ...
                <div v-if="showBatchUploadForm"
                    ...
                    <div class="space-y-4">
                        <!-- 文件上传 -->
                        <div>
                            <label for="markdown-files" class="block text-sm font-medium mb-1">
                                选择Markdown文件夹/文件 <span class="text-red-500">*</span>
                            </label>
                            <div class="flex items-center">
                                <label for="markdown-files"
                                    class="flex-1 cursor-pointer border border-dashed border-[var(--color-border)] rounded-md p-4 text-center hover:border-github-green transition-colors">
                                    <input type="file" id="markdown-files" class="hidden" accept=".md,text/markdown"
                                        @change="handleMultipleFilesChange" multiple directory webkitdirectory />
                                    <div v-if="batchFiles.length === 0" class="flex flex-col items-center">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-10 w-10 text-github-gray mb-2"
                                            fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" />
                                        </svg>
                                        <span class="text-github-gray">点击选择文件夹或多个文件</span>
                                        <span class="text-xs text-github-gray mt-1">支持选择整个文件夹或多个文件</span>
                                    </div>
                                    <div v-else class="flex flex-col items-center text-github-green">
                        ...
                        <!-- 分类 -->
                        <div>
                            <label for="batch-category" class="block text-sm font-medium mb-1">分类 (应用于所有文件)</label>
                            <input type="text" id="batch-category" v-model="batchCategory" placeholder="分类名称"
                                class="w-full p-2 border border-[var(--color-border)] rounded-md bg-[var(--color-bg-primary)]" />
                            <div class="mt-1 flex flex-wrap gap-2" v-if="blogStore.categories.length">
                                <span class="text-xs">现有分类:</span>
                                <button v-for="category in blogStore.categories" :key="category"
                        ...
                        <!-- 标签 -->
                        <div>
                            <label for="batch-tags" class="block text-sm font-medium mb-1">标签 (用逗号分隔，应用于所有文件)</label>
                            <input type="text" id="batch-tags" v-model="batchTags" placeholder="标签1, 标签2, 标签3"
                                class="w-full p-2 border border-[var(--color-border)] rounded-md bg-[var(--color-bg-primary)]" />
                            <div class="mt-1 flex flex-wrap gap-2" v-if="blogStore.tags.length">
                                <span class="text-xs">现有标签:</span>
                                <button v-for="tag in blogStore.tags" :key="tag"
                                    @click="batchTags ? (batchTags.includes(tag) ? null : batchTags += `, ${tag}`) : batchTags = tag"
                                    class="px-2 py-1 text-xs rounded-full bg-blue-100 text-github-blue dark:bg-blue-900 dark:text-blue-300 hover:bg-blue-200 dark:hover:bg-blue-800 transition-colors">
                                    {{ tag }}
                                </button>
                            </div>
                        ...
                        <!-- 提交按钮 -->
                        <div class="flex justify-end gap-2 mt-6">
                            <button @click="cancelBatchUpload"
                                class="px-4 py-2 border border-[var(--color-border)] rounded-md hover:bg-gray-100 dark:hover:bg-gray-800">
                                取消
                            </button>
                            <button @click="handleBatchUpload"
                                :disabled="batchFiles.length === 0 || uploadStatus.loading" :class="[
                                    'px-4 py-2 rounded-md text-white',
                                    batchFiles.length === 0 || uploadStatus.loading ? 'bg-gray-400 cursor-not-allowed' : 'bg-github-green hover:bg-green-700'
                                ]">
                ...
                <div class="mb-6 flex flex-col space-y-4">
                    ...
                    <div class="flex flex-wrap gap-2">
                        ...
                        <button v-if="categoryFilter || tagFilter || searchQuery"
                            @click="categoryFilter = ''; tagFilter = ''; searchQuery = ''"
